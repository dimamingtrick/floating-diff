...
WebStorm-style Git for VS Code and Cursor: diffs in a floating window, a branches popup, a Git Log with a commit graph, browsing a branch without checking it out, and a GitStorm panel in the Activity Bar.
...
- Setting `gitStorm.openFromSourceControl` (default on): clicks in Source Control open the floating window. This relies on internals of the built-in Git extension; if a VS Code update changes them, clicks open tabs as usual and the icon keeps working.
...
- `⑂ Branches` in the status bar (bottom left) opens the panel next to the terminal and closes it again; `Git Log` (bottom right) and `GitStorm: Git Log` open it too.
...
- **File History** — the clock button in an editor's title (or `GitStorm: File History`) opens the panel with the history of that file, on the branch picked on the left: click another branch to see the file's history there without checking it out.
...
- The panes resize by dragging their edges. `GitStorm: Git Log in Editor` shows the log in an editor tab instead.
- The branch name in the GitStorm panel opens the branch list to switch branches.

## Browse a branch without checkout

- `GitStorm: Browse Branch…`, the eye button of a branch in the GitStorm panel, or `Browse files at this branch` in the branches popup.
...
## Build and install
    ...
    npm install
    npm run package
    code --install-extension gitstorm-0.4.0.vsix
    cursor --install-extension gitstorm-0.4.0.vsix
