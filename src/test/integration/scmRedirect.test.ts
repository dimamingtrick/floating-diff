...
describe('Source Control redirect', () => {
	it('makes clicking a changed file in Source Control open the floating window', async function () {
		...
		await waitFor(() => changed()?.command.command === 'gitConvenient.openScmResource', 'Source Control click redirected', 15000);
	...
	it('does the same for a new (untracked) file', async function () {
		...
		await waitFor(() => untracked()?.command.command === 'gitConvenient.openScmResource', 'untracked click redirected', 15000);
