...
export async function pickChange(diffWindow: DiffWindow): Promise<void> {
	...
	if (!api) {
		void vscode.window.showErrorMessage('Git Convenient: the built-in Git extension is disabled.');
		return;
	}
	const items = listChanges(api);
	if (items.length === 0) {
		void vscode.window.showInformationMessage('Git Convenient: no changes.');
		return;
	}
