...
- **Right** — the files of the selected commit as a tree with file counts (click selects a file, double-click or `Enter` opens its diff in the floating window), then the message, hash, author, date and the branches that contain it; Open diff, Cherry-pick, Revert and more stay at the bottom.
